...
if(isset($_REQUEST['payment-id']))
{
	...
	//var_dump($response);
	$note = mysqli_real_escape_string($con,serialize($response));
	$order_id = intval($response['data']['transaction']['order']['order_id']);
	if(is_array($response) && !empty($response)) {
		if($response['status'] && $response['data']['transaction']['status'] == "Success") {
			$sql = "update billing SET status='Active',note='" . $note . "' WHERE billing_id='" . $order_id ."'";
			 mysqli_query($con,$sql);
			//echo "Transaction success";
			echo "<script>window.location='billingreceipt.php?paymentid=$order_id';</script>";
		} else {
...
if(isset($_POST['submit']))
{
	...
	{ 
		...
		if(mysqli_affected_rows($con) == 1)
		{
			...
			//echo "<script>window.location='submitpaykun.php?productid=$insid';</script>";
		}
	}
}
if(isset($_GET['editid']))
...
<!-- about -->
		...
		if(isset($_REQUEST['payment-id']))
		{
			if($response['status'] && $response['data']['transaction']['status'] == "Success") 
			{
				echo "<center><img src='img/vfmov3.gif'></centeR>";
			} 
			else
			{
				echo "<center><img src='img/failed.png'></centeR>";
				echo "<script>alert('Transaction failed');</script>";
				echo "<script>window.location='selectcategory.php';</script>";
			}
		}
		else
		{
		?>
			<div class="checkout-left">	
				...
				<div class="col-md-8 ">
<form action="submitpaykun.php" method="post" onsubmit="return validateform()" class="creditly-card-form agileinfo_form" enctype="multipart/form-data">
...
<label class="control-label"><h3>Upload <span>Product</span></h3></label>
		...
		<?php
		$sqlcategory = "select * from category WHERE status='Active' AND category_id='" . $_SESSION['categoryid'] ."'";
...
<button  type="submit" name="submitproduct"  class="form-control" >Submit</button>
