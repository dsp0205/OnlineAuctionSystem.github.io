...
if(isset($_REQUEST['payment-id']))
{
	...
	if(is_array($response) && !empty($response)) {
		if($response['status'] && $response['data']['transaction']['status'] == "Success") {
			...
			echo "<script>window.location='paymentreceipt.php?paymentid=$order_id';</script>";
		} else {
...
if(isset($_POST['submit']))
{
	...
	if(mysqli_affected_rows($con) == 1)
	{
		...
		//echo "<SCRIPT>window.location='submit.php?paymentid=$paymentid';</SCRIPT>";
	}
}
...
<!-- about -->
		...
		<hr>
			...
			<center><h3>Deposit <span>Form</span></h3></center>
			<div class="row">	

				<div class="col-md-3"></div>
				<div class="col-md-6">

				<form action="submitpaykun.php" method="post" class="creditly-card-form agileinfo_form" onsubmit="return validateform()">
...
<div class="w3_agileits_card_number_grid_left">
	<div class="controls">
		...
		<input name="paid_amount" id="paid_amount" class="form-control" type="number" placeholder="Deposit amount" value="<?php echo $rsedit['paid_amount']; ?>">
	</div>
</div>
